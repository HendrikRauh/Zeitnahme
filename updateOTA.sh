# Hilfsfunktion: OTA-Update für ein Gerät und Typ wiederholen
retry_ota_update() {
    local ssid="$1"
    ...
    while [ $try -le $max_retries ]; do
        ...
        if ip addr show "$WIFI_IFACE" | grep -q '192\.168\.4\.'; then
            ...
            if [ "$type" = "fw" ]; then
                ...
                if [ "${STATUS_FW[$ssid_hash]}" = "🟢" ]; then success=1; break; fi
            elif [ "$type" = "fs" ]; then
                run_ota_update "$ESP_IP" "$clean_ssid_name"
                if [ "${STATUS_FS[$ssid_hash]}" = "🟢" ]; then success=1; break; fi
            fi
        else
...
main() {
    ...
    # 1. Durchlauf: alle Updates versuchen
    for ssid in "${ESP_SSIDS[@]}"; do
        clean_ssid_name=$(clean_ssid "$ssid")
    ...
    # 2. Durchlauf: fehlgeschlagene Updates sammeln und wiederholen
    declare -a retry_fw
    declare -a retry_fs
    for ssid in "${ESP_SSIDS[@]}"; do
        clean_ssid_name=$(clean_ssid "$ssid")
